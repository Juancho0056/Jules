<script lang="ts">
  ...
  const sessionStoreUnsubscribe = sessionStore.subscribe(value => {
    ...
    if (currentIsAuthenticated && !previousIsAuthenticated) { // Just logged in
        updateQueueStatus(); 
        if (!statusInterval) {
             statusInterval = setInterval(updateQueueStatus, 5000);
        }
    } else if (!currentIsAuthenticated && previousIsAuthenticated) { // Just logged out
  ...
  const updateQueueStatus = async () => {
    if (!currentIsAuthenticated) { // Guard against running if not authenticated
  ...
  onMount(() => {
    ...
    if (currentIsAuthenticated) {
        updateQueueStatus();
        if (!statusInterval) { // Ensure interval is started if authenticated
            statusInterval = setInterval(updateQueueStatus, 5000);
        }
    }
  });
  ...
  const handleManualSync = async () => {
    if (currentOfflineState.isOffline) { // Use the reactive variable
...
{#if currentIsAuthenticated} <!-- Only show indicator if authenticated -->
<div class="sync-indicator fixed bottom-4 left-4 p-3 bg-gray-700 text-white text-xs rounded-lg shadow-xl z-50 max-w-xs">
  <div class="flex justify-between items-center mb-1">
    ...
    <button title="Force Health Check" on:click={handleManualHealthCheck} class="ml-2 p-1 bg-blue-500 hover:bg-blue-600 rounded text-xs leading-none">Probe</button>
  </div>

  {#if $queueStatus}
    {#if $queueStatus.total > 0}
      <div class="mb-1">Pending Sync: {$queueStatus.pending + $queueStatus.failedRetryable} item(s)</div>
    {/if}
    {#if $queueStatus.failedPermanent > 0}
      <div class="text-red-400 mb-1">Failed Permanently: {$queueStatus.failedPermanent} item(s)</div>
    {/if}
    {#if $queueStatus.total === 0 && !currentOfflineState.isOffline} <!-- Use the reactive variable -->
       <div class="text-green-300 mb-1">Data synced.</div>
    {/if}
  {/if}
  
  {#if !currentOfflineState.isOffline} <!-- Use the reactive variable -->
    <button on:click={handleManualSync} class="w-full mt-1 p-1 bg-purple-500 hover:bg-purple-600 rounded text-xs">
      Sync Now
    </button>
  {:else}
    <button class="w-full mt-1 p-1 bg-gray-500 rounded text-xs cursor-not-allowed" disabled>
      Sync Now (Offline)
    </button>
  {/if}
</div>
