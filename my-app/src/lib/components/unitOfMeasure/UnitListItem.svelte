...
<li class="flex justify-between items-center p-3 border-b hover:bg-gray-50 transition-colors duration-150">
  <div class="flex-grow">
    <div class="flex items-center">
      <span class="font-semibold text-gray-700">{unit.name}</span>
      <span class="text-sm text-gray-500 ml-2">({unit.symbol} - {unit.codigo})</span>
      {#if isUnsynced}
      ...
      {#if !hasServerId && !isUnsynced} 
        ...
        <span title="Synced but server ID missing?" class="ml-2 text-red-600">(Error: Synced but no Server ID)</span>
      {/if}
    </div>
    <p class="text-xs text-gray-400 mt-1">
      Local ID: {unit.localId} | Server ID: {unit.id || 'N/A'} | Last Mod: {new Date(unit.fechaModificacion).toLocaleDateString()}
    </p>
  </div>
