<script lang="ts">
  ...
  import { unitOfMeasureStore, type UnitOfMeasureState } from '$lib/stores/unitOfMeasureStore'; // unitOfMeasureStore uses Dbo internally
  ...
  const handleDelete = async (event: CustomEvent<DeleteEventDetail>) => {
    if (confirm(`Are you sure you want to delete unit with code '${event.detail.codigo}'? This action will be synced.`)) {
      // Call store's remove with localId and codigo
      await unitOfMeasureStore.remove(event.detail.localId, event.detail.codigo);
...
{#if $unitOfMeasureStore.isLoading && $unitOfMeasureStore.units.length === 0} <!-- Show loading only if units are empty initially -->
  <p class="text-center text-gray-500 py-4">Loading units...</p>
{:else if $unitOfMeasureStore.error}
  <p class="text-center text-red-500 py-4">Error loading units: {$unitOfMeasureStore.error.message}</p>
{:else if $unitOfMeasureStore.units.length === 0}
  <p class="text-center text-gray-500 py-4">No units of measure found. Add one to get started!</p>
{:else}
  <ul class="list-none p-0 m-0 bg-white shadow-md rounded-lg">
    {#each $unitOfMeasureStore.units as unit (unit.localId)} <!-- Key by localId, which is Dexie's PK -->
