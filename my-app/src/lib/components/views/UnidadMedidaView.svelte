<script lang="ts">
  ...
  import { db, type UnitOfMeasureDbo } from "$lib/services/dbService";
  import {
    getAllUnidadMedida,
    addUnidadMedida,
    updateUnidadMedida,
  ...
  } from "$lib/services/unidadMedidaService";
  import {
  ...
  async function handleEdit(event: CustomEvent<UnitOfMeasureDbo>) {
  ...
  async function handleDelete(event: CustomEvent<UnitOfMeasureDbo>) {
    ...
    if (
      confirm(`Are you sure you want to delete "${unidadToDelete.nombre}"?`)
    ) {
      if (unidadToDelete.localId !== undefined) {
        await deleteUnidadMedida(unidadToDelete.localId);
        await loadUnidades();
      } else {
  ...
  async function handleSaveForm(event: CustomEvent<Record<string, any>>) {
    ...
    try {
      if (editingUnidad && editingUnidad.localId !== undefined) {
        // Update existing
        await updateUnidadMedida(editingUnidad.localId, {
...
<div class="view-container unidad-medida-view">
  {#if !$isOnline}
    <div class="offline-status-indicator" role="alert">
      ⚠️ You are currently offline. Data will be saved locally and synced when
      online.
    </div>
  {/if}
...
<style>
  ...
  .btn-primary {
    /* Basic button styling, can be centralized */
  ...
  :global(.btn-edit) {
    /* Using :global if styles are in AdvancedTable, or define here */
