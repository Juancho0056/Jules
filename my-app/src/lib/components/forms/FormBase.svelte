    ...
    <div class="form-field-container">
      {#if field.type === "text"}
        <TextInput
          ...
          on:input={(e) =>
            handleInputChange(field.name, (e.target as HTMLInputElement).value)}
      ...
      {:else if field.type === "textarea"}
        <div class="form-control">
          <label for={field.name} class="label"
            >{field.label}{#if field.required}<span class="required-asterisk"
                >*</span
              >{/if}</label
          >
          <textarea
            ...
            aria-describedby={formErrors[field.name]
              ? `${field.name}-error`
              : undefined}
          ></textarea>
          {#if formErrors[field.name]}
            <p class="error-message" id={`${field.name}-error`}>
...
<style>
  ...
  .form-control {
    /* Copied from TextInput for consistency for direct textarea */
    width: 100%;
    margin-bottom: 1rem;
  }
  .label {
