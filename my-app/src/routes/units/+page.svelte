<script lang="ts">
  ...
  const toggleAddNewForm = () => {
    if ($unitOfMeasureStore.selectedUnitToEdit) {
  ...
  $: showAnyForm = showFormForNew || !!$unitOfMeasureStore.selectedUnitToEdit;
  // Determine which unit to pass to the form: null for new, or the selected one for editing.
  $: currentUnitForForm = showFormForNew ? null : $unitOfMeasureStore.selectedUnitToEdit;
...
<div class="container mx-auto p-6 bg-gray-50 min-h-screen">
  <div class="flex justify-between items-center mb-6">
    ...
    <button 
      ...
      {#if showFormForNew}
        Cancel New Unit
      {:else if $unitOfMeasureStore.selectedUnitToEdit}
  ...
  {#if $offlineStore.isOffline}
    <p class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4 rounded-md shadow" role="alert">
      You are currently offline. Operations will be queued and synced when online.
    </p>
  {/if}
  ...
  {#if $unitOfMeasureStore.error && $unitOfMeasureStore.units.length === 0 }
    <p class="text-center text-red-600 py-4">Error loading units: {$unitOfMeasureStore.error.message || 'Unknown error'}</p>
  {/if}

  {#if showAnyForm}
    <div class="mb-8 p-6 bg-white rounded-lg shadow-xl">
      <UnitForm unitToEdit={currentUnitForForm} on:formSubmitted={handleFormSubmitted} on:formCleared={handleFormCleared} />
    </div>
  {/if}
  ...
  <div class="mt-8 p-4 border border-gray-200 rounded-md bg-gray-50">
      ...
      <div class="flex space-x-2">
        <button on:click={() => offlineStore.setOfflineMode(true)} class="text-sm bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-3 rounded-md shadow-sm">Go Offline</button>
        <button on:click={() => offlineStore.setOfflineMode(false)} class="text-sm bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-3 rounded-md shadow-sm">Go Online</button>
        <button on:click={() => syncService.fetchAllUnidadesMedida()} class="text-sm bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-3 rounded-md shadow-sm">Force Refresh Units</button>
        <button on:click={() => syncService.processQueue()} class="text-sm bg-purple-500 hover:bg-purple-600 text-white font-semibold py-2 px-3 rounded-md shadow-sm">Force Process Queue</button>
      </div>
  </div>
</div>
