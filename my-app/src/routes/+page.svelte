<script lang="ts">
  ...
  const toggleForm = () => {
    showForm = !showForm;
    if (!showForm) {
      // If closing form, ensure unitToEdit is cleared
      unitToEdit = null;
    }
  };
...
<div class="container mx-auto p-4">
  ...
  {#if $offlineStore.isOffline}
    <p
      ...
      role="alert"
    >
      You are currently offline. Some actions may be queued and synced when you
      are back online.
    </p>
  {/if}
  ...
  <div class="mt-6 p-2 border rounded">
    ...
    <button
      ...
      class="text-xs bg-orange-500 text-white p-1 rounded mr-1"
      >Go Offline</button
    >
    <button
      on:click={() => offlineStore.setOfflineMode(false)}
      class="text-xs bg-green-500 text-white p-1 rounded mr-1">Go Online</button
    >
    <button
      on:click={() => unitOfMeasureStore.fetchAll()}
      class="text-xs bg-blue-500 text-white p-1 rounded"
